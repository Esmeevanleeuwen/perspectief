...
export default async function NewResearchPage({ searchParams }: Props) {
  ...
  return (
    <div className="admin-page admin-create">
      <Link href="/admin/onderzoeken" className="text-xs text-[var(--muted)] underline-offset-4 hover:underline">← Onderzoeken</Link>
      <div className="mt-5 border-b border-[#102534]/10 pb-7"><p className="text-[0.68rem] uppercase tracking-[0.2em] text-[var(--accent)]">Nieuw onderzoek</p><h1 className="mt-2 font-serif text-5xl tracking-[-0.04em]">Nieuw onderzoek</h1><p className="mt-3 max-w-xl text-sm leading-6 text-[var(--muted)]">Na het aanmaken bouw je het onderzoek verder op met leesblokken en kun je het als uitgelicht onderzoek op de homepage zetten.</p></div>
      ...
      <form action={createResearch} className="mt-7 grid gap-6 rounded-md border border-[#102534]/10 bg-[var(--paper)] p-5 md:p-7">
        ...
        <label className="grid gap-2 text-xs font-medium">Slug <span className="font-normal text-[var(--muted)]">Optioneel; wordt anders uit de titel gemaakt.</span><input name="slug" className={field} /></label>
        ...
        <label className="grid gap-2 text-xs font-medium">Dimensies <span className="font-normal text-[var(--muted)]">Eén per regel</span><textarea name="dimensions" rows={4} className={`${field} resize-y`} /></label>
        ...
        <div className="flex justify-end border-t border-[#102534]/10 pt-5"><button className="rounded-md bg-[var(--accent)] px-5 py-3 text-xs font-medium text-white">Onderzoek aanmaken →</button></div>
      </form>
    </div>
  );
}
