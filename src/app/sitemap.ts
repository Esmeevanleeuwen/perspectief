...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [articles, reports] = await Promise.all([readCatalogAll("meridian"), readReportCatalog("meridian")]);
  const entries = [
    ...[...new Set(paths)].filter(path => !path.startsWith("/artikelen/")).map(path => ({url:absoluteUrl(path)})),
    ...articles.filter(a => a.seo.indexable && articleOrigin(a) && (!a.canonical_url || a.canonical_url === `${articleOrigin(a)}/artikelen/${a.slug}`)).map(a => ({url:`${articleOrigin(a)}/artikelen/${a.slug}`,lastModified:a.updated_at})),
    ...reports.filter(r => r.indexable && articleOrigin(r)).map(r => ({url:`${articleOrigin(r)}/verslagen/${r.slug}`,lastModified:r.updated_at})),
  ];
