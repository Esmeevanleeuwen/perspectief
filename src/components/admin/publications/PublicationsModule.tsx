...
export default function PublicationsModule({ filters, deleted = false }: { filters: Filters; deleted?: boolean }) {
  return (
    <AdminModuleLayout
      accent="#63469c"
      header={<AdminPageHeading title="Publicaties" description="Beheer de verhalen en artikelen van Meridian."><PublicationActions /></AdminPageHeading>}
    ...
    >
      <Suspense key={JSON.stringify(filters)} fallback={<AdminModuleState kind="loading">Publicaties laden…</AdminModuleState>}>
        <PublicationsData filters={filters}>
          {result => <PublicationsResults result={result} filters={filters} columns={publicationColumns} basePath="/admin/content" />}
        </PublicationsData>
      </Suspense>
    </AdminModuleLayout>
  );
