  ...
  const shownPage = {
    ...
    items: search.result.items.map((item) =>
      docs.dirty.includes(item.key) && docs.documents[item.key]
        ? {
            ...item,
            title: docs.documents[item.key].title,
            summary:
              docs.documents[item.key].summary ||
              docs.documents[item.key].sections
    ...
    <div
      onKeyDown={(event) => {
        if (
          ...
          if (capture)
            event.currentTarget
              .querySelector<HTMLFormElement>(".writing-capture")
              ?.requestSubmit();
          else void save();
        }
      }}
      ...
      <WorkspaceLayout
          ...
          <Collections
            ...
            onCreate={(name) => {
              ...
              if (state.collections.length >= 100) {
                setMessage("Je kunt maximaal 100 mappen bewaren.");
                return;
              }
        ...
        tools={
          <div className="writing-list-tools">
            ...
            <WritingMenu
              className="writing-open-menu"
              label="⋯"
              ariaLabel="Open teksten en sessies"
            >
              <DocumentTabs
                view={state.view}
              ...
              />
              <div className="writing-new-publication">
                <PublicationActions />
              </div>
            </WritingMenu>
          </div>
            ...
            <div className="writing-document-nav">
              ...
              {!capture && active && (
                <span className="writing-muted">
                  {state.view.mode === "reference" ? "Teksten vergelijken" : ""}
                </span>
              )}
            </div>
            <QuickCapture
