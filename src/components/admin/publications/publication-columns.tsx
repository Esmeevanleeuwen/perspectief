...
import PublicationStatus from "@/components/admin/publications/PublicationStatus";
...
export const publicationColumns: readonly AdminColumn<Publication>[] = [
  ...
  { id: "status", label: "Status", render: item => item.canChangeStatus ? <PublicationStatus key={`${item.id}:${item.updatedAt}`} item={item} /> : <span className="admin-status" data-status={item.status}>{optionLabel(publicationStatuses, item.status)}</span> },
