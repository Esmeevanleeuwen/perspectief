...
export const publicationColumns: readonly AdminColumn<Publication>[] = [
  ...
  { id: "actions", label: "Acties", render: item => <div className="admin-row-actions"><Link href={`/admin/werkplek?open=${encodeURIComponent(`publication:${item.id}`)}`}>Schrijven</Link><Link href={`/admin/content/${item.id}`}>Instellingen</Link>{item.status === "published" && <Link href={publicContentHref(item.type, item.slug)} target="_blank" rel="noopener noreferrer">Bekijk ↗</Link>}</div> },
];
