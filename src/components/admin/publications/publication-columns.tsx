...
/** Reorder, remove or add columns here; no query or layout changes needed. */
export const publicationColumns: readonly AdminColumn<Publication>[] = [
  ...
  { id: "actions", label: "Acties", render: item => <div className="admin-row-actions"><Link href={`/admin/content/${item.id}`}>Bewerken</Link>{item.status === "published" && <Link href={publicContentHref(item.type, item.slug)} target="_blank" rel="noopener noreferrer">Bekijk ↗</Link>}</div> },
];
