...
import { articleStructuredData, articleBreadcrumbData } from "@/lib/publishing/seo";
...
export default function SharedArticleView({ article }: { article: SharedArticle }) {
  const jsonld = [articleStructuredData(article), articleBreadcrumbData(article)].filter(Boolean);
  ...
  return <main className="pub-article">
    ...
    <nav aria-label="Kruimelpad"><Link href="/">{article.platform === "avera" ? "Amparis" : "Meridian"}</Link> / <Link href="/artikelen">Artikelen</Link>{article.report && <> / <Link href={`/verslagen/${article.report.slug}`}>{article.report.title}</Link></>} / <span aria-current="page">{article.title}</span></nav>
    <article>
      <header><p>{article.eyebrow || "Artikel"}</p><h1>{article.title}</h1>
        ...
        <small>Gepubliceerd <time dateTime={article.published_at}>{new Date(article.published_at).toLocaleDateString("nl-NL", { timeZone: "Europe/Amsterdam" })}</time>{Date.parse(article.updated_at) - Date.parse(article.published_at) > 60000 && <> · Bijgewerkt <time dateTime={article.updated_at}>{new Date(article.updated_at).toLocaleDateString("nl-NL", { timeZone: "Europe/Amsterdam" })}</time></>}</small>
      </header>
      {image && <img className="pub-hero" fetchPriority="high" decoding="async" src={image} alt={article.image_alt || ""} />}
