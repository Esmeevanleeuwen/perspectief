...
export function relatedDossiers(current: DossierSummary, all: DossierSummary[], limit = 4) {
    ...
    return all.filter((item) => item.slug !== current.slug).map((item) => {
        const shared = [...new Map(item.themes.filter((theme) => currentThemes.has(topicSlug(theme))).map(theme => [topicSlug(theme), theme])).values()];
        return { ...item, shared, score: shared.length / Math.max(1, new Set([...currentThemes, ...item.themes.map(topicSlug)]).size) };
    }).filter((item) => item.shared.length > 0)
        .sort((a, b) => b.score - a.score || a.slug.localeCompare(b.slug)).slice(0, limit);
}
export function uniqueChapters(chapters: DossierChapter[]): DossierChapter[] {
    const seen = new Set<string>();
    return chapters.filter((chapter) => {
        if (!chapter.id || !chapter.title || seen.has(chapter.id) || !chapter.paragraphs.some((text) => text.trim()))
            return false;
        seen.add(chapter.id);
        return true;
    });
...
export const chapterPath = (slug: string, chapter: string) => `${dossierPath(slug)}/hoofdstukken/${encodeURIComponent(chapter)}`;
export const sourcePath = (slug: string) => `/bronnen/${encodeURIComponent(slug)}`;
export const pageAnchor = (documentSlug: string, number: number) => `document-${documentSlug}-page-${number}`;
