...
const local: Dossier[] = research.map(item => ({ slug: item.slug, title: item.title, description: item.summary,
    themes: ["Bestuur", "Transparantie"], status: "Doorlopend onderzoek", indexable: true,
    question: item.question, method: item.method, boundaries: "Onderzoeksvragen, ervaringen en verklaringen zijn geen automatische feitenvaststelling. Ontbrekende bronnen blijven zichtbaar.",
    chapters: uniqueChapters(item.sections.map(section => ({ id: section.id, title: section.title, paragraphs: [...(section.intro ? [section.intro] : []), ...section.paragraphs], points: section.points }))),
    articles: getArticlesForResearch(item.slug).map(article => ({ title: article.title, description: article.description, href: `/artikelen/${article.slug}` }))
}));
const publishedRows = cache(async (): Promise<Record<string, unknown>[]> => {
    ...
    const { data, error } = await supabase.from("content_items")
        ...
        .eq("content_type", "research").eq("status", "published").order("published_at", { ascending: false });
    if (error)
        throw new Error("Openbare onderzoeksdossiers konden niet worden geladen.");
    return records(data);
});
function summary(row: Record<string, unknown>): DossierSummary {
    const known = local.find(d => d.slug === row.slug);
    return { slug: text(row.slug), title: text(row.title), description: text(row.summary), themes: known?.themes ?? [], status: "Gepubliceerd onderzoek", indexable: true };
}
export const getDossiers = cache(async (): Promise<DossierSummary[]> => {
    ...
    for (const row of rows)
        if (text(row.slug) && text(row.title))
            all.set(text(row.slug), summary(row));
    return [...all.values()];
});
export const getDossier = cache(async (slug: string): Promise<Dossier | undefined> => {
    const row = (await publishedRows()).find(row => row.slug === slug);
    if (!row)
        return [...local, pilotDossier].find(d => d.slug === slug);
    ...
    for (const section of sections) {
        ...
        if (section.section_type === "heading" || title || !grouped.length)
            grouped.push({ id: text(section.id), title: title || (section.section_type === "heading" ? body : text(row.title)), paragraphs: [] });
    ...
    for (const link of records(row.content_sources)) {
        ...
        if (source.visibility === "private" || (typeof source.status === "string" && !["published", "verified"].includes(source.status))) continue;
        try {
            const url = new URL(text(source.url));
            if (["https:", "http:"].includes(url.protocol) && !url.username && !url.password && text(source.title))
                links.push({ title: text(source.title), href: url.href, description: "Bronverwijzing bij dit onderzoek. Controleer de inhoud en context bij de oorspronkelijke uitgever." });
        } catch {}
    }
    let articleLinks: DossierLink[] = [];
    const supabase = publicClient();
    if (supabase && text(row.id)) {
        const { data, error } = await supabase.from("research_children")
            .select("relation,content_items!child_content_id(slug,title,summary,content_type,status)").eq("research_content_id", text(row.id));
        if (error) throw new Error("Dossierverbindingen konden niet worden geladen.");
        articleLinks = records(data).flatMap(relation => {
            ...
            return [{ title: text(child.title), description: text(child.summary), href: `/artikelen/${encodeURIComponent(text(child.slug))}` }];
        });
    }
    return { ...summary(row), question: text(meta.central_question) || text(row.summary), method: text(meta.method),
        boundaries: text(meta.boundaries) || "Controleer de bronverwijzingen en onderscheid bevindingen, interpretaties en open vragen.",
        chapters, articles: articleLinks, externalSources: links };
});
...
export const getArticleDossiers = cache(async (slug: string): Promise<DossierSummary[]> => {
    const dossiers = await Promise.all((await getDossiers()).map(d => getDossier(d.slug)));
    return dossiers.filter((d): d is Dossier => Boolean(d?.articles.some(a => a.href === `/artikelen/${encodeURIComponent(slug)}`)));
});
